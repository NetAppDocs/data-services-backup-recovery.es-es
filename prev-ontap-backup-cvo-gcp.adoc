...
---
= Haz una copia de seguridad de Cloud Volumes ONTAP en Google Cloud Storage con NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utiliza NetApp Backup and Recovery para hacer copias de seguridad de los datos de los volúmenes de tus sistemas Cloud Volumes ONTAP en Google Cloud Storage.
...
También necesitas una cuenta de Google Cloud para el espacio de almacenamiento donde se ubicarán tus backups.
...
Para usar la funcionalidad de búsqueda y restauración de NetApp Backup and Recovery, el rol para el agente de la Console debe incluir permisos específicos para que pueda acceder al servicio BigQuery de Google Cloud. Consulta los permisos a continuación y sigue los pasos si necesitas modificar la política.
...
+
.. Selecciona *Editar proyecto* y elige un nuevo proyecto si el que deseas utilizar es distinto del proyecto predeterminado (donde se encuentra el agente de la Console).
...
. *Servicios*: Deja activado NetApp Backup and Recovery y selecciona *Continuar*.
...
.. En *Teclas de acceso para cuentas de servicio*, selecciona *Crear una clave para una cuenta de servicio*, selecciona la cuenta de servicio que acabas de crear y selecciona *Crear clave*.
+
...
+
Si el destino de Google Cloud Storage para tus copias de seguridad aparece como un sistema en la página *Sistemas* de la NetApp Console, puedes arrastrar el clúster ONTAP hasta el sistema de Google Cloud Storage.
...
** Para las políticas de backup a objetos, configura Datalock y la protección contra ransomware. Para detalles sobre Datalock y la protección contra ransomware, consulta link:prev-ontap-policy-object-options.html["Configuración de la política de copia de seguridad en objeto"].
** Seleccione *Crear*.
...
+
Si decides utilizar tus propias claves gestionadas por el cliente, introduce la información del llavero y de la clave.
