...
---
= Haz una copia de seguridad de Cloud Volumes ONTAP en Azure Blob storage con NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utiliza NetApp Backup and Recovery para hacer copias de seguridad de los datos de los volúmenes de tus sistemas Cloud Volumes ONTAP en Azure Blob storage.
...
También necesitas una suscripción a Azure para el espacio de almacenamiento donde estarán ubicados tus backups.
...
Para usar la funcionalidad de búsqueda y restauración de NetApp Backup and Recovery, el rol para el agente de la Console debe incluir permisos específicos para que pueda acceder al espacio de trabajo de Azure Synapse y a la cuenta de Azure Data Lake Storage. Consulta los permisos a continuación y sigue los pasos si necesitas modificar la política.
...
Puedes habilitar NetApp Backup and Recovery en un sistema Cloud Volumes ONTAP nuevo o existente. Los pasos difieren ligeramente para cada opción.
...
.Pasos
. En la página *Sistemas* de la NetApp Console, selecciona el sistema y selecciona *Habilitar* junto a Backup and Recovery en el panel derecho.
+
...
Activa las copias de seguridad en cualquier momento directamente desde tu sistema Cloud Volumes ONTAP.
...
+
La página de introducción del asistente muestra las opciones de protección, incluyendo instantáneas locales, replicación y backups. Si usaste la segunda opción en este paso, aparece la página de definición de estrategia de backup con un volumen seleccionado.
...
Selecciona los volúmenes que deseas proteger. Un volumen protegido es aquel que tiene una o más de las siguientes: política de snapshot, política de replicación, política de backup a objeto.
...
NOTE: Si has elegido una cuenta de almacenamiento de Azure ya existente, la información de cifrado ya está disponible, por lo que no es necesario que la introduzcas ahora.
...
. Si lo deseas, marca la casilla para *sincronizar automáticamente las etiquetas de la política de snapshot con las etiquetas de las políticas de replicación y backup*. Esto crea snapshots con una etiqueta que coincide con las etiquetas de las políticas de replicación y backup.
...
Aparece el panel de control de backup de volúmenes para que puedas supervisar el estado de los backups.
