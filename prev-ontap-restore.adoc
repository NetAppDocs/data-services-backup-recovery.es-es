...
Antes de restaurar, anote el nombre del volumen de origen, el sistema de origen y el SVM, y la fecha del archivo de copia de seguridad. Puede restaurar datos de ONTAP desde una instantánea, un volumen replicado o desde copias de seguridad almacenadas en almacenamiento de objetos.


NOTE: Si el archivo de copia de seguridad que desea restaurar reside en un almacenamiento en la nube de archivo (a partir de ONTAP 9.10.1), la operación de restauración tarda más y genera un costo.  El clúster de destino también debe ejecutar ONTAP 9.10.1 o superior para la restauración de volúmenes, 9.11.1 para la restauración de archivos, 9.12.1 para Google Archive y StorageGRID, y 9.13.1 para la restauración de carpetas.
...
. Para restaurar una carpeta o archivos desde una copia de seguridad de volumen, seleccione el menú *Restaurar* y, a continuación, seleccione *Restaurar archivos o carpeta* en _Examinar y restaurar_.
...
La operación de búsqueda examina todas las instantáneas locales que existen para sus volúmenes ONTAP , todos los volúmenes replicados en sistemas de almacenamiento secundarios y todos los archivos de copia de seguridad que existen en el almacenamiento de objetos.  Dado que restaurar datos desde una instantánea local o un volumen replicado puede ser más rápido y menos costoso que restaurarlos desde un archivo de copia de seguridad en el almacenamiento de objetos, es posible que desee restaurar datos desde estas otras ubicaciones.
