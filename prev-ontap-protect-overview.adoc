...
---
= Aprende sobre NetApp Backup and Recovery para volúmenes de ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* Si planeas escanear un archivo de backup en busca de ransomware durante el proceso de restauración de datos de volumen (si has activado la protección Datalock y Ransomware para tus backups en la nube), también incurrirás en costes de salida adicionales de tu proveedor de nube.
...
Si tu clúster está usando ONTAP 9.11.1 o superior, puedes proteger tus backups en el almacenamiento de objetos contra el borrado y ataques de ransomware. Cada política de backup ofrece una sección para _Datalock y protección contra ransomware_ que puedes aplicar a tus archivos de backup durante un periodo de tiempo específico: el _periodo de retención_.
