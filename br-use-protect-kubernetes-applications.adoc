...
[role="lead"]
NetApp Backup and Recovery te permite añadir aplicaciones de Kubernetes a través de la interfaz web o aplicando archivos de recursos personalizados. Las aplicaciones pueden estar basadas en espacios de nombres y compuestas por recursos estándar de Kubernetes, o basadas en máquinas virtuales y compuestas por una o más máquinas virtuales.
...
El primer paso para proteger las aplicaciones de Kubernetes es crear una aplicación en NetApp Backup and Recovery. Cuando creas una aplicación, haces que Backup and Recovery reconozca la aplicación que se está ejecutando en el clúster de Kubernetes.
...
NOTE: Si seleccionas VMs de la lista, la definición de la aplicación es estática: las nuevas VMs no se añaden a la aplicación después (tienes que editar la app para agregarlas y protegerlas). Si usas selectores de etiquetas, no puedes elegir VMs individuales ni editar la lista generada, pero cualquier VM que después coincida con el selector se incluye automáticamente y se protege.
