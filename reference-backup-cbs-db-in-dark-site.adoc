...
summary: 'En modo privado (sin acceso a internet), NetApp Backup and Recovery almacena sus datos de configuración en el mismo bucket StorageGRID u ONTAP S3 que usas para tus copias de seguridad. Si el sistema host del agente de la consola falla, puedes implementar un nuevo agente de la consola y restaurar estos datos.' 
---
...
[role="lead"]
En modo _privado_ (sin acceso a Internet), NetApp Backup and Recovery almacena sus datos de configuración en el mismo bucket de StorageGRID u ONTAP S3 utilizado para tus copias de seguridad. Si el sistema host del agente de la consola falla, puedes implementar un nuevo agente de la consola y restaurar estos datos.

También puedes seguir estos pasos para sustituir el agente de la consola y restaurar los datos de configuración que se almacenaban en él al utilizar Backup and Recovery en _modo estándar_ en las instalaciones, cuando no intervienen cuentas en la nube (por ejemplo, al utilizar NetApp StorageGRID como destino de la copia de seguridad).
...
Cuando instales un nuevo agente de la Consola, descarga la misma versión de software que el agente original. Los cambios en la base de datos NetApp Backup and Recovery pueden hacer que las versiones más recientes del software no funcionen con copias de seguridad antiguas de la base de datos. Puedes https://docs.netapp.com/us-en/console-setup-admin/task-upgrade-connector.html["Actualice el software del agente de la consola a la versión más actual después de restaurar la base de datos de respaldo"^].
...
. Inicia sesión en NetApp Console usando las credenciales del usuario admin que acabas de crear.
...
. Copia la copia de seguridad de MySQL desde la ubicación de backup al nuevo host del agente de Console. El siguiente ejemplo usa el nombre de archivo `CBS_DB_Backup_23_05_2023.sql`.
...
----
. Asegúrese de que los volúmenes que se muestran sean los mismos que existían en su entorno original.
...
. Copia el archivo ZIP de la copia de backup del catálogo indexado desde la ubicación de la copia de backup a la carpeta  `/opt/application/netapp/cbs` en el nuevo host del agente de NetApp Console. El siguiente ejemplo utiliza el nombre de archivo  `Indexed_Catalog_DB_Backup_catalogdb1_23_05_2023.zip`.
...
----
. Ejecuta el comando *ls* para asegurarte de que se ha creado la carpeta "catalogdb1" y que contiene las subcarpetas `files`, `versions` y `snapshots`.
...
Agrega los detalles del sistema StorageGRID que está asociado con tus sistemas ONTAP, tal como se configuraron en la configuración original del agente de la Console, usando el  https://docs.netapp.com/us-en/console-automation/index.html["API de la NetApp Console"^].
...
----
+
Aunque los valores de la dirección IP, el nombre de usuario y la contraseña son personalizados, el nombre de la cuenta no lo es. El nombre de la cuenta es siempre `account-DARKSITE1`. Además, el nombre de usuario debe usar un nombre con formato de correo electrónico.

+
Esta API devuelve una respuesta como la siguiente. Puedes recuperar el token de autorización tal y como se muestra a continuación.
...
----
+
Esta API devuelve una respuesta como la siguiente. El valor que aparece en `resourceIdentifier` corresponde al _ID del entorno de trabajo_, y el valor que aparece en `agentId` corresponde al _x-agent-id_.
...
----
. Actualiza la base de datos de NetApp Backup and Recovery con los detalles del sistema StorageGRID que está asociado con los sistemas. Asegúrate de ingresar el nombre de dominio completo de StorageGRID, junto con la clave de acceso y la clave de almacenamiento, como se muestra a continuación:
+
[source, shell]
----
...
. Selecciona cada sistema ONTAP y selecciona *Ver backups* junto al servicio de Backup and Recovery en el panel derecho.
+
Debería ver todas las copias de seguridad creadas para sus volúmenes.

. Desde el panel de control de Restore, en la sección Search & Restore, selecciona *Indexing Settings*.
+
Asegúrate de que los sistemas en los que se había activado previamente la catalogación indexada sigan activados.
