...
[role="lead"]
Con NetApp Backup and Recovery, restaura los datos de ONTAP usando Browse & Restore. Antes de restaurar, toma nota del nombre del volumen de origen, el sistema de origen, la máquina virtual de almacenamiento y la fecha del archivo de copia de seguridad. Puedes restaurar datos de ONTAP desde una instantánea, un volumen replicado o desde copias de seguridad almacenadas en un almacén de objetos.
...
| StorageGRID en NetApp | Sistema ONTAP local  a| 
* Sistema ONTAP local
* Cloud Volumes ONTAP
...
+
Ten en cuenta que si seleccionas un archivo de backup en el almacenamiento de objetos y la protección contra ransomware está activa para ese backup (si activaste Datalock y la protección contra ransomware en la política de backup), entonces se te pedirá que ejecutes un análisis adicional de ransomware en el archivo de backup antes de restaurar los datos. Te recomendamos que analices el archivo de backup en busca de ransomware. (Vas a incurrir en costes de salida adicionales de tu proveedor de nube para acceder al contenido del archivo de backup.)
...
+
Y si la protección contra ransomware está activa para el archivo de backup (si activaste Datalock y la protección contra ransomware en la política de backup), entonces se te pedirá que ejecutes un análisis adicional de ransomware en el archivo de backup antes de restaurar los datos. Te recomendamos que analices el archivo de backup en busca de ransomware. (Vas a incurrir en costes de salida adicionales de tu proveedor de nube para acceder al contenido del archivo de backup.)
