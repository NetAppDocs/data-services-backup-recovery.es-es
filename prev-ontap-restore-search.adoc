...
---
= Restaura datos de ONTAP usando Buscar y restaurar en NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utiliza la función Search and Restore para recuperar volúmenes, carpetas o archivos de los backups de ONTAP cuando no sepas el nombre exacto del sistema, volumen o archivo. Puedes buscar en snapshots locales, volúmenes replicados y backups de almacenamiento de objetos.
...
* Cuando restauras un volumen habilitado para SnapLock, el volumen restaurado conserva la protección SnapLock solo si el almacenamiento de destino está habilitado para WORM. Si restauras en un destino que no está habilitado para WORM, se elimina el estado SnapLock del volumen restaurado.


====
Antes de empezar, deberías tener una idea del nombre o la ubicación del volumen o archivo que quieres restaurar.
...
** Debes configurar un LIF de datos NFS IPv4 en la máquina virtual de almacenamiento y una exportación NFS para el volumen. Search & Restore admite tanto volúmenes NFS como SMB/CIFS.
** El servidor RPC de SnapDiff debe estar activado en la máquina virtual de almacenamiento. La Console lo hace automáticamente cuando habilitas la indexación en el sistema. (SnapDiff es la tecnología que identifica rápidamente las diferencias de archivos y directorios entre snapshots.)
...
* Requisitos de Azure:
+
** Debes registrar el proveedor de recursos de Azure Synapse Analytics (denominado «Microsoft.Synapse») en tu suscripción. https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/resource-providers-and-types#register-resource-provider["Vea cómo registrar este proveedor de recursos para su suscripción"^]. Debes ser el *Owner* o *Contributor* de la suscripción para registrar el proveedor de recursos.
...
NOTE: Backup and Recovery no indexa ninguna instantánea cuya marca de hora de creación coincida exactamente con otra instantánea del mismo volumen.
...
.Pasos para habilitar la indexación en un sistema:
Si la indexación ya está habilitada para su sistema, vaya a la siguiente sección para restaurar sus datos.

En primer lugar, monta un volumen independiente para almacenar los archivos del catálogo. Esto ayuda a evitar la pérdida de datos si los archivos del catálogo de instantáneas se vuelven demasiado grandes. No es necesario hacerlo en todos los clústeres; puedes montar cualquier volumen de cualquiera de los clústeres de tu entorno. Si no lo haces, es posible que la indexación no funcione correctamente.
...
[[mount-volume-steps]]
.Paso para montar el volumen para la reindexación del catálogo
...
.. En el área «Filtrar por», selecciona los criterios de filtrado. Por ejemplo, puedes seleccionar el sistema donde residen los datos y el tipo de archivo, como un archivo .JPEG. También puedes seleccionar el tipo de ubicación de backup si quieres buscar solo dentro de las snapshots disponibles o archivos de backup en el almacenamiento de objetos.
...
+
Ten en cuenta que los resultados identifican las instantáneas de volúmenes locales y los volúmenes replicados remotos que contienen el archivo en tu búsqueda. Puedes elegir restaurar desde el archivo de backup en la nube, la instantánea o el volumen replicado.
