...
.. *Configuración de KVM*: Haz lo siguiente:
+
... Si aún no has guardado credenciales para este host KVM, selecciona *Add credentials*.
+
...
... Agrega un nuevo host KVM introduciendo la siguiente información:
+
**** *FQDN o dirección IP de KVM*: ingrese el FQDN o la dirección IP del host.
**** *Agentes*: Elige el agente de Console que se utilizará para la comunicación entre Backup and Recovery y el host KVM.
**** *Credenciales*: selecciona una credencial guardada para el host KVM.




.. *Configuración avanzada*: si lo deseas, introduce un puerto para comunicarte con el host KVM y selecciona la plataforma de gestión del host de la lista.
