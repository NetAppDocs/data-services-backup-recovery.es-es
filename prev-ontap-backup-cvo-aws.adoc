...
---
= Realiza un backup de Cloud Volumes ONTAP en Amazon S3 con NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utiliza NetApp Backup and Recovery para hacer backup de los datos de los volúmenes de tus sistemas Cloud Volumes ONTAP en Amazon S3.
...
NOTE: Si utilizas un punto de conexión VPC privado para las transferencias de datos de backup o restauración, este tiene prioridad sobre cualquier conexión de proxy configurada.
...
También necesitas una cuenta de AWS para el espacio de almacenamiento donde estarán ubicados tus backups.
...
.Pasos
. En la página *Sistemas* de la Console, selecciona el clúster y selecciona *Habilitar* junto a Backup and Recovery en el panel derecho.
+
Si el destino de Amazon S3 para tus copias de seguridad aparece como un sistema en la página *Sistemas*, puedes arrastrar el clúster hasta el sistema de Amazon S3 para iniciar el asistente de configuración.
...
+
Si el destino de Amazon S3 para tus copias de seguridad aparece como un sistema en la página *Systems* de la Console, puedes arrastrar el clúster ONTAP hasta el sistema de Amazon S3.

** Selecciona *Volúmenes* en la barra de Backup and Recovery. En la pestaña Volúmenes, selecciona el icono *Acciones* image:icon-action.png["Icono de acciones"] y selecciona *Activar protección 3-2-1* para un único volumen (que aún no tenga habilitada la replicación ni el backup en almacén de objetos).
...
+
Si decides utilizar tus propias claves gestionadas por el cliente, introduce la información de la clave.
...
. Si lo deseas, marca la casilla *Corregir automáticamente las etiquetas que no coincidan en la instantánea local, la replicación y el backup*. Esto crea instantáneas con una etiqueta que coincide con las etiquetas de las políticas de instantánea, replicación y backup.
