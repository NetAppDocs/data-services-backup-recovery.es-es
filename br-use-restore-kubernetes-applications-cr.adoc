...
---
= Restaura aplicaciones de Kubernetes con recursos personalizados
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utiliza recursos personalizados para restaurar tus aplicaciones a partir de una instantánea o un backup. Restaurar desde una instantánea existente suele ser más rápido cuando restauras la aplicación en el mismo clúster.
...
* Puedes usar la configuración avanzada para personalizar las operaciones de restauración. Para obtener más información, consulta link:br-use-kubernetes-advanced-restore-settings.html["Configuración avanzada de restauración para Kubernetes en NetApp Backup and Recovery"].
...
* Cuando utilices un CR para restaurar en un nuevo espacio de nombres, debes crear manualmente el espacio de nombres de destino antes de aplicar el CR. NetApp Backup and Recovery crea automáticamente espacios de nombres solo cuando usas la CLI.
...
NOTE: Si el BackupRestore CR no está disponible, puedes usar el comando del paso 2 para ver el contenido del backup.
