...
---
= Realiza copias de seguridad de los datos de Cloud Volumes ONTAP en almacenamiento compatible con S3 con NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Completa algunos pasos en NetApp Backup and Recovery para empezar a hacer copias de seguridad de los datos de volumen de tus sistemas Cloud Volumes ONTAP en S3.
...
Permisos necesarios de AWS Cloud Volumes ONTAP:: Cuando tu sistema Cloud Volumes ONTAP está ejecutando el software ONTAP 9.12.1 o superior, la función de IAM que le otorga permisos a ese sistema debe incluir un nuevo conjunto de permisos compatibles con S3 específicamente para NetApp Backup and Recovery desde la última https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-set-up-iam-roles.html["Política de Cloud Volumes ONTAP"^].
+
--
...
+
Si el destino compatible con S3 para tus copias de seguridad existe como clúster en la página *Sistemas*, puedes arrastrar el clúster al sistema compatible con S3 para iniciar el asistente de configuración.
...
. *Copia de seguridad*: Establezca las siguientes opciones:
+
...
** *Exportar instantánea existente*: si hay instantáneas locales para volúmenes en este sistema que coinciden con la etiqueta de programación de respaldo que acaba de seleccionar para este sistema (por ejemplo, diaria, semanal, etc.), se muestra este mensaje adicional. Marque esta casilla para que todas las instantáneas históricas se copien en el almacenamiento de objetos como archivos de respaldo para garantizar la protección más completa para sus volúmenes.
...
. Opcionalmente, marque la casilla para *Reparar automáticamente las etiquetas no coincidentes en la instantánea local, la replicación y la copia de seguridad*. Esto crea instantáneas con una etiqueta que coincide con las etiquetas de las políticas de instantáneas, replicación y copia de seguridad.
