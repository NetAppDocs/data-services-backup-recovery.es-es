...
[role="lead"]
En NetApp Backup and Recovery, puedes hacer copias de seguridad de los datos de los volúmenes de tus sistemas ONTAP locales en un sistema de almacenamiento secundario y en Google Cloud Storage. Usa esta página para preparar tu entorno y activar las copias de seguridad.
...
* Ten en cuenta que si estás usando un IPspace diferente al Default, puede que necesites crear una ruta estática para acceder al almacén de objetos.
...
+
Si el destino de Google Cloud Storage para tus copias de seguridad aparece en la página *Systems* de la Console, puedes arrastrar el clúster ONTAP a Google Cloud Storage.
...
** *Clave de cifrado*: Si has creado un nuevo depósito de Google Cloud Storage, introduce la información de la clave de cifrado que te haya facilitado el proveedor. Elige si vas a utilizar las claves de cifrado predeterminadas de Google Cloud o si vas a utilizar tus propias claves gestionadas por el cliente desde tu cuenta de Google Cloud para gestionar el cifrado de tus datos.
+

NOTE: Si has elegido un bucket existente de Google Cloud Storage, la información sobre el cifrado ya está disponible, así que no necesitas introducirla ahora.
