...
. Revise el estado de las opciones de protección en la columna "Protección existente". Los 3 iconos representan "Instantáneas locales", "Volúmenes replicados" y "Copias de seguridad en almacenamiento de objetos".
+
...
.Pasos
. Desde la pestaña *Volúmenes*, identifique el volumen en el que desea activar las copias de seguridad, seleccione el menú Acciones image:icon-action.png["Icono de acciones"] al final de la fila y seleccione *Activar protección 3-2-1*.
. En la página _Definir estrategia de copia de seguridad_, seleccione la arquitectura de copia de seguridad y, a continuación, defina las políticas y otros detalles para las instantáneas locales, los volúmenes replicados y los archivos de copia de seguridad. Consulte los detalles de las opciones de respaldo de los volúmenes iniciales que activó en este sistema. Luego, seleccione *Siguiente*.
...
+
Si habilitaste _Datalock and Ransomware protection_ para backups en la nube en la política de backup inicial al activar NetApp Backup and Recovery para este clúster, solo verás otras políticas que se hayan configurado con DataLock. Y si no habilitaste _Datalock and Ransomware protection_ al activar NetApp Backup and Recovery, solo verás otras políticas de backup en la nube que no tengan DataLock configurado.
...
Si habilitaste _Datalock y protección contra ransomware_ al activar NetApp Backup and Recovery para este clúster, la copia de seguridad a petición también se configurará con DataLock y el período de retención será de 30 días. Los análisis de ransomware no son compatibles con las copias de seguridad a petición. link:prev-ontap-policy-object-options.html["Obtenga más información sobre DataLock y la protección contra ransomware"^].
...
Esta función solo está disponible si la copia de seguridad del volumen se creó desde un sistema con ONTAP 9.11.1 o superior, y si activaste _Datalock and Ransomware protection_ en la política de backup a objeto.
...
[NOTE]
====
* Si habilitaste _Datalock and Ransomware protection_ en la política inicial al activar NetApp Backup and Recovery para este clúster, cualquier política que edites debe configurarse con la misma configuración de DataLock (Governance o Compliance). Y si no habilitaste _Datalock and Ransomware protection_ al activar NetApp Backup and Recovery, no puedes habilitar DataLock ahora.
...
[NOTE]
====
* Si habilitaste _Datalock and Ransomware protection_ en la política inicial al activar NetApp Backup and Recovery para este clúster, cualquier política adicional que crees debe configurarse con el mismo ajuste DataLock (Governance o Compliance). Y si no habilitaste _Datalock and Ransomware protection_ al activar NetApp Backup and Recovery, no puedes crear nuevas políticas que usen DataLock.
...
.Pasos
. En el menú *Volúmenes*, haz clic en image:icon-actions-horizontal.gif["Icono más"] para el volumen de origen y selecciona *Gestionar copias de seguridad > Ver copia de seguridad*.
+
