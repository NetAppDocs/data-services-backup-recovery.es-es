...
Para más detalles sobre los requisitos de protección Datalock y Ransomware para StorageGRID, consulta link:prev-ontap-policy-object-options.html["Opciones de política de copia de seguridad a objeto"].
...
+
Si tu cluster utiliza ONTAP 9.11.1 o superior, puedes elegir proteger tus backups de borrado y ataques de ransomware configurando _Datalock and Ransomware protection_. _DataLock_ protege tus archivos de backup para que no sean modificados ni borrados, y _ransomware protection_ escanea tus archivos de backup para buscar evidencia de un ataque de ransomware en tus archivos de backup.
