...
[role="lead"]
En NetApp Backup and Recovery, puedes hacer copias de seguridad de los datos de los volúmenes de tus sistemas ONTAP locales principales en ONTAP S3 y, opcionalmente, en un sistema ONTAP secundario para replicación. Usa esta página para preparar tu entorno y activar las copias de seguridad.
...
. *Instantánea local*: Elige una política de snapshot ya existente o crea una nueva.
+
...
+
** *Destino de la replicación*: Selecciona el sistema de destino y la máquina virtual de almacenamiento. Si lo deseas, selecciona el agregado de destino (o los agregados para los volúmenes FlexGroup) y un prefijo o sufijo que se añadirá al nombre del volumen replicado.
