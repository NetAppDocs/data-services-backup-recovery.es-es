...
---
= Agrega y protege aplicaciones de Kubernetes en NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Usa la interfaz web de NetApp Backup and Recovery o archivos de recursos personalizados para agregar aplicaciones de Kubernetes para protección. Puedes agregar aplicaciones basadas en namespaces que usen recursos estándar de Kubernetes o aplicaciones basadas en máquinas virtuales que usen una o más máquinas virtuales.
...
Para proteger una aplicación de Kubernetes, primero crea la aplicación en NetApp Backup and Recovery. Cuando creas la aplicación, NetApp Backup and Recovery detecta la aplicación en ejecución en el clúster de Kubernetes.
...
[role="tabbed-block"]
====
.Agrega una aplicación basada en namespace (interfaz web)
--
.Pasos
. En NetApp Backup and Recovery, seleccione *Inventario*.
. En la parte superior derecha de la página, asegúrate de que *Kubernetes* esté seleccionado en la lista de cargas de trabajo.
. Selecciona *Ver* en la línea de la carga de trabajo para ver los recursos de Kubernetes.
. Seleccione la pestaña *Aplicaciones*.
. Seleccione *Crear aplicación*.
. Introduzca un nombre para la aplicación.
. En la lista *Cluster*, selecciona el cluster que aloja la aplicación.
. En *Filtros*, elige *Espacio de nombres* para filtrar las aplicaciones por espacio de nombres.
...
+
... Selecciona *Add cluster-scoped resources* para añadir recursos con alcance a nivel de cluster. Si los incluyes, se añaden a la aplicación cuando la creas.
... Opcionalmente, seleccione *Buscar* para encontrar los recursos según sus criterios de búsqueda.
+

NOTE: Backup and Recovery no almacena los parámetros de búsqueda ni los resultados; los parámetros se utilizan para buscar en el clúster de Kubernetes seleccionado los recursos que se pueden incluir en la aplicación.





. NetApp Backup and Recovery muestra una lista de recursos que coinciden con tus criterios de búsqueda.
. Si la lista contiene los recursos que desea proteger, seleccione *Siguiente*.
. Opcionalmente, en el área *Policy*, elige una política de protección existente para proteger la aplicación o crea una nueva política. Si no seleccionas una política, la aplicación se crea sin política de protección. Puedes link:br-use-policies-kubernetes-create.html#create-a-policy["añadir una política de protección"] más adelante.
...
.Resultado
La aplicación se crea y aparece en la lista de aplicaciones de la pestaña *Applications* del inventario de Kubernetes. Backup and Recovery habilita la protección de la aplicación en función de tu configuración, y puedes supervisar el progreso en el área *Monitoring*.

--
.Agrega una aplicación basada en VM (interfaz web)
--
.Pasos
. En NetApp Backup and Recovery, seleccione *Inventario*.
. En la parte superior derecha de la página, asegúrate de que *Kubernetes* esté seleccionado en la lista de cargas de trabajo.
...
. Introduzca un nombre para la aplicación.
. En la lista *Cluster*, selecciona el cluster que aloja la aplicación.
. En *Filtros*, elige *Máquinas virtuales* para crear una aplicación basada en VM.
. Busca máquinas virtuales para añadir a la aplicación eligiendo un espacio de nombres y, opcionalmente, incluyendo selectores de etiquetas.
+

NOTE: Si seleccionas máquinas virtuales de la lista, la definición de la aplicación es estática: no se añaden nuevas máquinas virtuales a la aplicación posteriormente (debes editar la aplicación para añadirlas y protegerlas). Si utilizas selectores de etiquetas, no puedes seleccionar máquinas virtuales individuales ni editar la lista generada, pero cualquier máquina virtual que posteriormente se ajuste al selector se incluye automáticamente y queda protegida.

+
Las máquinas virtuales seleccionadas aparecen en la lista de la derecha.

. Si la lista contiene las máquinas virtuales que quieres proteger, selecciona *Siguiente*.
. Opcionalmente, en el área *Policy*, elige una política de protección existente para proteger la aplicación o crea una nueva política. Si no seleccionas una política, la aplicación se crea sin política de protección. Puedes link:br-use-policies-kubernetes-create.html#create-a-policy["añadir una política de protección"] más adelante.
. En el área *Prescripts y postscripts*, habilite y configure cualquier gancho de ejecución de prescript o postscript que desee ejecutar antes o después de las operaciones de respaldo.  Para habilitar prescriptos o posscriptos, debe haber creado previamente al menos unolink:br-use-manage-execution-hook-templates.html["plantilla de gancho de ejecución"] .
. Seleccione *Crear*.
...
.Resultado
La aplicación se crea y aparece en la lista de aplicaciones de la pestaña *Aplicaciones* del inventario de Kubernetes. NetApp Backup and Recovery permite proteger la aplicación según tu configuración, y puedes supervisar el progreso en el área *Supervisión*.

--
.Añade una aplicación basada en el espacio de nombres (CR)
--
.Pasos
