...
[role="lead"]
Usa NetApp Backup and Recovery para gestionar las copias de seguridad de instantáneas, la replicación y las copias de seguridad en almacén de objetos para tus sistemas Cloud Volumes ONTAP y ONTAP locales. Puedes cambiar los horarios, activar o desactivar las copias de seguridad de volúmenes, pausar copias de seguridad, eliminarlas y dar de baja el servicio.
...
. Comprueba el estado de las opciones de protección en la columna «Protección existente». Los tres iconos representan «Instantáneas locales», «Volúmenes replicados» y «Copias de seguridad en almacén de objetos».
+
...
Consulta link:br-start-enable.html["Habilita NetApp Backup and Recovery para proteger volúmenes"^] para obtener instrucciones.
...
.Pasos
. Desde el menú *Volúmenes*, localice el volumen para el cual desea modificar la configuración de política, seleccione el menú Acciones image:icon-action.png["Icono de acciones"] al final de la fila y seleccione *Editar estrategia de respaldo*.
. En la página _Editar estrategia de copia de seguridad_, realice cambios en las políticas de copia de seguridad existentes para instantáneas locales, volúmenes replicados y archivos de copia de seguridad y seleccione *Siguiente*.
+
Si activaste la _protección DataLock y contra ransomware_ para las copias de seguridad en la nube en la política de backup inicial al activar NetApp Backup and Recovery para este clúster, solo verás otras políticas que se hayan configurado con DataLock. Y si no habilitaste la _protección DataLock y contra ransomware_ al activar NetApp Backup and Recovery, solo verás otras políticas de backup en la nube que no tengan configurado DataLock.
...
Cuando se habilitan más de cinco volúmenes para la copia de seguridad, NetApp Backup and Recovery inicializa solo cinco volúmenes a la vez. Cuando estos están terminados, se continúa en grupos de 5 hasta que se inicializan todos los volúmenes.
...
. Dependiendo del tipo de acción de copia de seguridad que quieras configurar, selecciona el botón en el menú Acciones masivas:
+
[cols="50,30"]
|===
...
|===
. En la página de copia de seguridad que aparece, realice cambios en las políticas de copia de seguridad existentes para instantáneas locales, volúmenes replicados o archivos de copia de seguridad y seleccione *Guardar*.
+
Si activaste la _protección DataLock y contra ransomware_ para las copias de seguridad en la nube en la política de backup inicial al activar NetApp Backup and Recovery para este clúster, solo verás otras políticas que se hayan configurado con DataLock. Y si no habilitaste la _protección DataLock y contra ransomware_ al activar NetApp Backup and Recovery, solo verás otras políticas de backup en la nube que no tengan configurado DataLock.
...
Si habilitaste _DataLock y protección contra ransomware_ al activar NetApp Backup and Recovery para este clúster, la copia de seguridad bajo demanda también se configurará con DataLock, y el periodo de retención será de 30 días. Los análisis de ransomware no son compatibles con las copias de seguridad ad hoc. link:prev-ontap-policy-object-options.html["Obtenga más información sobre DataLock y la protección contra ransomware"^].

Cuando creas un backup ad hoc, se crea una snapshot en el volumen de origen. Como esta snapshot no forma parte de una programación normal de snapshots, no se eliminará automáticamente. Quizá quieras eliminar manualmente esta snapshot del volumen de origen una vez que el backup haya finalizado. Esto permitirá liberar los bloques relacionados con esta snapshot. El nombre de la snapshot comenzará con `cbs-snapshot-adhoc-`. https://docs.netapp.com/us-en/ontap/san-admin/delete-all-existing-snapshot-copies-volume-task.html["Descubre cómo eliminar una instantánea usando la CLI de ONTAP"^].
...
Esta función solo está disponible si la copia de seguridad del volumen se ha creado desde un sistema con ONTAP 9.11.1 o una versión posterior, y si habilitaste _DataLock and Ransomware protection_ en la política de backup a objeto.
...
. Selecciona image:icon-actions-horizontal.gif["Icono de acciones"] para el archivo de copia de seguridad del volumen que quieres analizar en busca de ransomware y selecciona *Analizar en busca de ransomware*.
+
La columna de protección contra ransomware muestra que el análisis está en curso.
...
[NOTE]
====
* Si activaste la _protección DataLock y contra ransomware_ en la política inicial al activar NetApp Backup and Recovery para este clúster, cualquier política que edites debe configurarse con el mismo ajuste de DataLock (Governance o Compliance). Y si no activaste la _protección DataLock y contra ransomware_ al activar NetApp Backup and Recovery, no puedes activar DataLock ahora.
...
+
link:prev-reference-aws-archive-storage-tiers.html["Obtenga más información sobre el uso del almacenamiento de archivo de AWS"]. link:prev-reference-azure-archive-storage-tiers.html["Obtenga más información sobre el uso del almacenamiento de archivo de Azure"]. link:prev-reference-gcp-archive-storage-tiers.html["Obtenga más información sobre el uso del almacenamiento de archivos de Google"]. (Requiere ONTAP 9.12.1.)
...
[NOTE]
====
* Si habilitaste la _protección DataLock y contra ransomware_ en la política inicial al activar NetApp Backup and Recovery para este clúster, cualquier política adicional que crees debe configurarse con el mismo ajuste de DataLock (Governance o Compliance). Y si no habilitaste la _protección DataLock y contra ransomware_ al activar NetApp Backup and Recovery, no puedes crear nuevas políticas que usen DataLock.
...
NOTE: No puedes eliminar archivos individuales de copia de seguridad de un volumen una vez que se ha eliminado la relación de copia de seguridad. Elimina los archivos de copia de seguridad de un volumen antes de eliminar la relación de copia de seguridad del volumen.

.Pasos
. En el menú *Volúmenes*, selecciona image:icon-actions-horizontal.gif["Icono más"] para el volumen de origen y selecciona *Gestionar copias de seguridad > Ver copia de seguridad*.
+
Se muestra la lista de todos los archivos de respaldo.

. Elige una copia de seguridad de la lista y selecciona *Acciones > Eliminar todas las copias de seguridad*.
. Confirma la eliminación introduciendo el nombre del volumen.
. Expande el área de *Configuración avanzada*.
. En *Forzar borrado de copias de seguridad*, selecciona si quieres forzar el borrado de todas las copias de seguridad.
+
En raras ocasiones, podrías necesitar revocar el acceso de NetApp Backup and Recovery a las copias de seguridad; por ejemplo, si Backup and Recovery pierde acceso al bucket de copias de seguridad o las copias de seguridad están protegidas por DataLock pero ya no las quieres. Usa esta opción para forzar la eliminación de copias de seguridad a nivel de volumen y sistema.

+

CAUTION: Usa esta opción solo para limpiezas extremas. Una vez activada, NetApp Backup and Recovery ya no accederá a estas copias de seguridad (aunque los objetos permanezcan en el almacenamiento en la nube). Debes eliminarlas manualmente de tu proveedor de nube.
...
Elimina una copia de seguridad individual cuando ya no la necesites. No puedes eliminar volúmenes replicados (volúmenes de protección de datos).


NOTE: No puedes eliminar archivos de copia de seguridad de volumen individuales una vez que se ha eliminado la relación de copia de seguridad. Sin embargo, puedes <<Eliminar todos los archivos de respaldo de un volumen,eliminar todos los archivos de copia de seguridad del volumen>>.
...
NOTE: No puedes eliminar archivos de copia de seguridad de volúmenes individuales una vez que se ha eliminado la relación de copia de seguridad. Sin embargo, puedes <<Eliminar todos los archivos de respaldo de un volumen,eliminar todos los archivos de copia de seguridad del volumen>>. Si necesitas eliminar una relación de copia de seguridad para un volumen, asegúrate de eliminar primero las copias de seguridad del volumen y luego elimina la relación de copia de seguridad.
...
La desactivación de NetApp Backup and Recovery para un sistema pausa las copias de seguridad y las restauraciones para todos sus volúmenes sin eliminar las copias de seguridad existentes. Esto te permite pausar toda la actividad de copia de seguridad y restauración durante un período de tiempo sin dar de baja el servicio de copia de seguridad de este sistema.
