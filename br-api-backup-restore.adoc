...
.Pasos
. Obtén un token de usuario del sitio web de NetApp Console.
+
Asegúrate de generar el token de actualización desde el siguiente enlace: https://services.cloud.netapp.com/refresh-token/. El token de actualización es una cadena alfanumérica que usarás para generar un token de usuario.
