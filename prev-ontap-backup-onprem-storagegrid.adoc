...
[role="lead"]
En NetApp Backup and Recovery, puedes hacer copias de seguridad de los datos de los volúmenes de tus sistemas ONTAP locales principales en un sistema de almacenamiento secundario y en el almacén de objetos StorageGRID. Usa esta página para preparar tu entorno y activar las copias de seguridad.
...
+
La nueva versión de NetApp Backup and Recovery que incluye la posibilidad de programar y crear instantáneas y volúmenes replicados, además de crear backups en almacén de objetos, requiere que estés usando la versión 3.9.31 o superior del agente de la Console. NetApp recomienda que uses esta última versión para gestionar todos tus backups.
...
Para obtener más información sobre los requisitos de DataLock y la protección contra ransomware para StorageGRID, consulta link:prev-ontap-policy-object-options.html["Opciones de política de copia de seguridad a objeto"].

Versiones de StorageGRID compatibles:: Se admite StorageGRID 10.3 y versiones posteriores.
+
--
Para usar la protección DataLock & Ransomware para tus backups, tus sistemas StorageGRID deben estar ejecutando la versión 11.6.0.3 o superior.
...
Para utilizar el almacenamiento de archivos, es necesario tener acceso a la IP del nodo de administración.
...
+
Si tu clúster utiliza ONTAP 9.11.1 o una versión superior, puedes elegir proteger tus copias de seguridad contra el borrado y los ataques de ransomware configurando _DataLock y Ransomware protection_. _DataLock_ protege tus archivos de copia de seguridad para que no sean modificados ni eliminados, y _ransomware protection_ analiza tus archivos de copia de seguridad para buscar indicios de un ataque de ransomware en ellos.
