...
---
= Requisitos previos de importación de SnapCenter para NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Antes de importar recursos desde el SnapCenter Plug-in for Microsoft SQL Server en NetApp Backup and Recovery, asegúrate de que tu entorno cumple estos requisitos adicionales.
...
== Asegúrate de que el host cumpla los requisitos para el SnapCenter Plug-in

Para importar recursos desde el SnapCenter Plug-in for Microsoft SQL Server, asegúrate de que se cumplan los requisitos del host para instalar el SnapCenter Plug-in for Microsoft SQL Server.
...
** `1`: Las restricciones remotas de UAC están desactivadas. Las cuentas locales eluden el filtrado de tokens y tienen privilegios administrativos completos al acceder remotamente.
...
----
. Comprueba que el valor actual esté establecido en «1» en `LocalAccountTokenFilterPolicy` ejecutando:
+
[listing]
----
