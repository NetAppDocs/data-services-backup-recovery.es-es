...
[role="lead"]
Utiliza esta página para comprobar que tu configuración es compatible y para configurar copias de seguridad desde tus sistemas ONTAP locales principales a un almacenamiento de objetos compatible con S3 en NetApp Backup and Recovery.
...
.Pasos
. Accede al asistente «Activar Backup and Recovery» de una de las siguientes formas:
+
** En la página *Sistemas* de la Console, selecciona el sistema y selecciona *Enable > Backup Volumes* junto a Backup and Recovery en el panel derecho.
...
====
.Pasos
Ten en cuenta que si los volúmenes que elijas ya tienen aplicadas políticas de snapshot o de replicación, las políticas que selecciones después sobrescribirán esas políticas existentes.
...
TIP: Si deseas crear una política personalizada antes de activar la instantánea, puedes utilizar System Manager o el comando de la CLI de ONTAP `snapmirror policy create`. Consulta.
...
*** Introduzca el nombre de la póliza.
*** Seleccione hasta cinco horarios, normalmente de diferentes frecuencias.
*** Para las políticas de backup a almacén de objetos, configura los ajustes de DataLock y la protección contra ransomware. Para obtener más información sobre DataLock y la protección contra ransomware, consulta link:prev-ontap-policy-object-options.html["Configuración de la política de copia de seguridad en objeto"].
*** Seleccione *Crear*.


** *Exportar instantáneas existentes al almacenamiento de objetos como archivos de backup*: Si hay alguna instantánea local de los volúmenes de este sistema que coincida con la etiqueta de la programación de backup que acabas de seleccionar (por ejemplo, diaria, semanal, etc.), aparecerá este mensaje adicional. Marca esta casilla para que todas las instantáneas históricas se copien al almacenamiento de objetos como archivos de backup y así asegurar la protección más completa para tus volúmenes.
...
. Si lo deseas, marca la casilla para *sincronizar automáticamente las etiquetas de la política de snapshot con las etiquetas de las políticas de replicación y backup*. Esto crea snapshots con una etiqueta que coincide con las etiquetas de las políticas de replicación y backup. Si las políticas no coinciden, no se crearán backups.
...
Quizá te interese mostrar y, si lo deseas, copiar los comandos de la API usados en el asistente Activate Backup and Recovery. Quizá te interese hacerlo para automatizar la activación del backup en sistemas futuros.

.Pasos
. En el asistente «Activar Backup and Recovery», selecciona *Ver solicitud de API*.
