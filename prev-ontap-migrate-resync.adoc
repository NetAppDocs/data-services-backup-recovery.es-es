...
[role="lead"]
Usa SnapMirror to Cloud Resync en NetApp Backup and Recovery para mantener disponibles las copias de seguridad en la nube existentes después de migrar un volumen con SnapMirror Logical Replication (LRSE). Esta función permite que las copias de seguridad continúen sin una nueva línea base y está disponible a partir de ONTAP 9.16.1.

SnapMirror to Cloud Resync es compatible con las migraciones de FlexVol y FlexGroup y te ayuda a mantener la continuidad de backup en entornos híbridos y multinube.
...
Imagina que tienes un volumen local llamado Vol1a. Este volumen tiene tres instantáneas: S1, S2 y S3. Estas instantáneas son puntos de restauración. Vol1a se respalda en la nube usando SnapMirror to Cloud (SM-C), pero solo S1 y S2 están en el almacén de objetos.
...
NOTE: No utilices una política SnapMirror de tipo "Vault" para clonar un volumen de un ONTAP cluster antiguo a un ONTAP cluster nuevo o en una configuración de arquitectura en cascada. Las políticas Vault cambian el UUID de versión de la instantánea durante la transferencia. El UUID de versión de la instantánea se usa para determinar si existe una instantánea común entre el origen y el destino.
...
+
NetApp Backup and Recovery valida que cada volumen seleccionado para la resincronización comparta una instantánea común con el almacén de objetos. En concreto, la última instantánea almacenada en el almacén de objetos también debe existir en la instantánea local del nuevo volumen clonado. Esta comprobación garantiza que los volúmenes estén listos para la operación SnapMirror to Cloud Resync.
...
. Comprueba la elegibilidad del volumen. El sistema comprueba si los volúmenes son elegibles para la resincronización. Si un volumen no es elegible, significa que no se cumplieron los criterios comunes de snapshot.
+
