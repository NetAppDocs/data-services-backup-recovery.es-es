...
summary: 'Utiliza recursos personalizados (CR) para crear instantáneas y copias de seguridad bajo demanda de una aplicación de Kubernetes, estableciendo una línea de base o protegiendo los cambios más recientes en tus datos.' 
---
= Haz copias de seguridad de aplicaciones de Kubernetes ahora usando recursos personalizados en NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utiliza recursos personalizados (CR) para crear instantáneas y copias de seguridad bajo demanda de una aplicación de Kubernetes, estableciendo una línea de base o protegiendo los cambios más recientes en tus datos.
...
** *espec.reclaimPolicy*: (_Opcional_) Define qué ocurre con el AppArchive de una instantánea cuando se elimina el registro de control (CR) de la instantánea. Aunque se establezca en `Retain`, el registro de control (CR) de la instantánea se sigue eliminando; solo se conserva el AppArchive. Opciones válidas:
+
...
** *spec.dataMover*: (_Opcional, requerido para clusters migrados desde Trident Protect_) una cadena que indica qué herramienta de copia de seguridad usar para la operación de copia de seguridad. Si este cluster ha sido migrado a Backup and Recovery desde Trident Protect, el valor distingue mayúsculas de minúsculas y debe ser `CBS`.
...
| protect.trident.netapp.io/pvc-bind-timeout-sec | cadena | Tiempo máximo (en segundos) para esperar a que cualquier PersistentVolumeClaims (PVC) recién creado alcance la fase  `Bound` antes de que falle la operación. | "1200" (20 minutos) 
|===