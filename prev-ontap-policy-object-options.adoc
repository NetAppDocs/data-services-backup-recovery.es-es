...
[role="lead"]
Utiliza las políticas de backup a objeto en NetApp Backup and Recovery para definir programaciones, retención, DataLock, protección contra ransomware y configuración de archivado para tus sistemas ONTAP locales y Cloud Volumes ONTAP.
...
Cada política de backup incluye una sección de _Etiquetas y retención_ que puedes aplicar a tus archivos de backup. Ten en cuenta que la política de snapshot aplicada al volumen debe ser una de las políticas reconocidas por NetApp Backup and Recovery o no se crearán archivos de backup.
...
Ambas funciones protegen tus archivos de copia de seguridad para que siempre tengas un archivo de backup válido desde el que recuperar datos en caso de un intento de ataque de ransomware contra tus backups. También es útil para cumplir ciertos requisitos normativos donde las copias de seguridad deben estar bloqueadas y conservarse durante un periodo de tiempo determinado. Cuando la opción *DataLock and Ransomware protection* está activada, el bucket en la nube que se aprovisiona como parte de la activación de NetApp Backup and Recovery tendrá habilitado el bloqueo de objetos y la versión de objetos.
...
. Cuando crees una política, despliega la sección *DataLock y protección contra ransomware*.
. Elija una de las siguientes opciones:
+
** *Ninguno*: la protección DataLock y la protección contra ransomware están desactivadas.
** *Desbloqueado*: la protección DataLock y la protección contra ransomware están activadas. Los usuarios con permisos específicos pueden sobrescribir o eliminar los archivos de backup protegidos durante el periodo de retención.
** *Bloqueado*: la protección DataLock y la protección contra ransomware están activadas. Ningún usuario puede sobrescribir o eliminar archivos de backup protegidos durante el periodo de retención. Esto satisface el cumplimiento normativo completo.
...
| Cloud Volumes ONTAP en Google Cloud | Almacenamiento en la nube de Google 
...
* Para AWS:
+
** Tus clusters deben ejecutar ONTAP 9.11.1 o superior
...
* Para Azure:
+
** Tus clústeres deben estar ejecutando ONTAP 9.12.1 o superior
** El agente de consola se puede implementar en la nube o en sus instalaciones.
...
Al utilizar AWS, Azure o Google Cloud Storage, puedes trasladar los archivos de backup más antiguos a una clase de almacenamiento de archivo o a un nivel de almacenamiento más económico después de cierto número de días. También puedes elegir enviar tus archivos de backup al almacenamiento de archivo inmediatamente, sin que se guarden primero en el almacenamiento estándar en la nube. Solo tienes que introducir *0* en el campo «Archivar tras días» para enviar tu archivo de backup directamente al almacenamiento de archivo. Esto puede ser especialmente útil para usuarios que rara vez necesitan acceder a los datos de los backups en la nube o para quienes están reemplazando una solución de backup en cinta.
...
** En el caso de Azure, puede organizar en niveles las copias de seguridad más antiguas en el almacenamiento _Azure Archive_. link:prev-reference-azure-archive-storage-tiers.html["Obtenga más información sobre el almacenamiento de archivo de Azure"^].
