...
Al restaurar una copia de seguridad en la nube en un sistema Cloud Volumes ONTAP con ONTAP 9.13.0 o superior o en un sistema ONTAP local que ejecute ONTAP 9.14.1, tendrás la opción de realizar una operación de _restauración rápida_. La restauración rápida es ideal para situaciones de recuperación ante desastres en las que necesitas proporcionar acceso a un volumen lo antes posible. Una restauración rápida restaura los metadatos del archivo de copia de seguridad en un volumen en lugar de restaurar el archivo de copia de seguridad completo. La restauración rápida no se recomienda para aplicaciones sensibles a la latencia ni para el rendimiento, y no es compatible con copias de seguridad en almacenamiento archivado.
...
+
** Al restaurar desde Amazon S3, selecciona el IPspace en el clúster ONTAP donde residirá el volumen de destino, introduce la clave de acceso y la clave secreta del usuario que creaste para darle al clúster ONTAP acceso al bucket de S3 y, si quieres, elige un endpoint VPC privado para una transferencia de datos segura. Si usas un endpoint VPC privado, este tiene prioridad sobre cualquier conexión de proxy configurada.
...
. Si lo deseas, activa la opción *Dar prioridad a esta restauración frente a la cola de tareas existente* para darle mayor prioridad a esta operación de restauración. Cuando está activada, otras tareas en ejecución pueden pausarse o retrasarse para permitir que esta restauración continúe.
. Seleccione *Restaurar*.
+
Vuelves al panel de control de restauración para que puedas revisar el progreso de la operación de restauración.
