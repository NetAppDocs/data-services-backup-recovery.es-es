...
Ambas funciones protegen tus archivos de backup para que siempre tengas un archivo de backup válido del que recuperar datos en caso de un intento de ataque de ransomware a tus backups. También es útil para cumplir ciertos requisitos normativos donde los backups deben estar bloqueados y retenidos durante un periodo de tiempo determinado. Cuando la opción *Datalock and Ransomware protection* está activada, el bucket de cloud que se aprovisiona como parte de la activación de NetApp Backup and Recovery tendrá habilitado el bloqueo de objetos y el versionado de objetos.
...
. Cuando crees una política, amplía la sección *Datalock and Ransomware protection*.
