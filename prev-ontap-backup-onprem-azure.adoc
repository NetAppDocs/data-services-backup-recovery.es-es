...
[role="lead"]
En NetApp Backup and Recovery, puedes hacer copias de seguridad de los datos de los volúmenes de tus sistemas ONTAP locales en un sistema de almacenamiento secundario y en Azure Blob. Usa esta página para preparar tu entorno y activar las copias de seguridad.
...
+
NetApp Backup and Recovery es compatible con las regiones de Azure Government cuando el agente de la Console se implementa en la nube, no cuando se instala en tus instalaciones. Además, debes implementar el agente de la Console desde Azure Marketplace. No puedes implementar el agente de la Console en una región de Government desde el sitio web de NetApp Console SaaS.
...
+
** Para la licencia PAYGO de NetApp Backup and Recovery, necesitarás una suscripción a la  https://marketplace.microsoft.com/en-us/product/netapp.cloud-manager?tab=Overview["Oferta de NetApp Console de Azure Marketplace"^]. La facturación de NetApp Backup and Recovery se realiza a través de esta suscripción.
...
* Si estás utilizando un espacio IP diferente al predeterminado, puede que necesites crear una ruta estática para acceder al almacén de objetos.
...
+
Ten en cuenta que NetApp Backup and Recovery admite las _políticas de acceso de Azure_ como modelo de permisos. El modelo de permisos de _control de acceso basado en roles de Azure_ (Azure RBAC) no es compatible actualmente.
...
+
** En la página *Systems* de la NetApp Console, selecciona el sistema y selecciona *Enable > Backup Volumes* junto al servicio de Backup and recovery en el panel derecho.
+
...
+
La página de Introducción del asistente muestra las opciones de protección, incluyendo instantáneas locales, replicación y backups. Si hiciste la segunda opción en este paso, aparece la página Define Backup Strategy con un volumen seleccionado.
...
NOTE: Si seleccionas *Bring your own bucket*, asegúrate de que el contenedor que creas como destino de la copia de seguridad tiene el mismo nombre que la cuenta de almacenamiento Azure.
