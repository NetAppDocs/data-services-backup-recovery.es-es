...
[role="lead"]
En NetApp Backup and Recovery, puedes usar las políticas de backup predeterminadas o crear políticas personalizadas para tus volúmenes ONTAP. Las políticas controlan la frecuencia de backup, el momento en que se realiza y la retención.
...
Cuando utilices el asistente de activación para habilitar el servicio de Backup and Recovery para tus volúmenes, puedes elegir entre las políticas predeterminadas y cualquier otra política que ya exista en el sistema (Cloud Volumes ONTAP o sistema ONTAP local). Si quieres usar una política diferente a las políticas existentes, puedes crear la política antes o mientras usas el asistente de activación.
...
NetApp Backup and Recovery proporciona tres tipos de backups de datos de ONTAP: snapshots, replicaciones y backups en almacenamiento de objetos. Sus políticas residen en diferentes ubicaciones según la arquitectura que uses y el tipo de backup:
...
. En la página de gestión de políticas, selecciona *Crear política* > *Crear política de snapshot*.
...
.Pasos
. En la página de gestión de políticas, selecciona *Crear política* > *Crear política de replicación*.
...
.Pasos
. En la página de gestión de políticas, selecciona *Crear política* > *Crear política de backup*.
. En la sección Detalles de la política, especifique el nombre de la política.
. Especifique las etiquetas SnapMirror (máximo de 5) que indican la retención de cada etiqueta.
