...
---
= Limitaciones conocidas con NetApp Backup and Recovery para cargas de trabajo de SQL Server
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Consulta estas limitaciones antes de usar NetApp Backup and Recovery para cargas de trabajo de SQL Server.
...
No puedes asignar varias políticas que utilicen el mismo nivel de programación (por ejemplo, por horas, por días o por semanas) a un recurso. NetApp Backup and Recovery no importará esos recursos desde SnapCenter.
...
Si una política de SnapCenter se repite tanto en intervalos diarios como mensuales, NetApp Backup and Recovery no importará la política.

Por ejemplo, no puedes asociar una política diaria (con un intervalo de 7 días o menos, o de más de 7 días) a un recurso y también asociar una política mensual al mismo recurso.
...
=== No se han migrado las políticas de backup bajo demanda

NetApp Backup and Recovery no importa las políticas de backup bajo demanda de SnapCenter.
...
SnapCenter no asigna clústeres de almacenamiento ni máquinas virtuales de almacenamiento de recursos a hosts, pero NetApp Backup and Recovery sí lo hace. El clúster ONTAP local o la máquina virtual de almacenamiento no se asignarán a un host en las versiones preliminares de NetApp Backup and Recovery. Además, NetApp Console no es compatible con máquinas virtuales de almacenamiento.
