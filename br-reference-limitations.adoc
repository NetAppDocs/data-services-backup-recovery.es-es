...
[role="lead"]
Consulta estas limitaciones antes de usar NetApp Backup and Recovery para volúmenes ONTAP.
...
* Solo puedes seleccionar un volumen FlexGroup a la vez para la replicación. Necesitas activar las copias de seguridad por separado para cada volumen FlexGroup.
+
Los volúmenes FlexVol no tienen esa limitación: puedes seleccionar todos los volúmenes FlexVol de tu sistema y asignarles las mismas políticas de backup.
...
* Cuando conviertes un volumen FlexVol a un volumen FlexGroup o viceversa, el volumen convertido se considera un volumen nuevo; necesitas configurar la protección para el volumen convertido después de la conversión. Puedes restaurar backups creados antes de la conversión a un volumen diferente.
...
* No realices copias de seguridad de volúmenes de destino de SnapMirror que tengan instantáneas de retención a largo plazo habilitadas. Estas instantáneas se crean directamente en el destino y no se moverán al almacenamiento de objetos. Esta restricción se aplica incluso si la política de SnapMirror se cambia después para deshabilitar la retención a largo plazo.
...
+
** Las relaciones con las etiquetas SnapMirror `app_consistent` y `all_source_snapshot` no se respaldarán en la nube.
** Si creas copias locales de instantáneas en el volumen de destino de SnapMirror (independientemente de las etiquetas de SnapMirror utilizadas), estas instantáneas no se moverán a la nube como backups. Debes crear una política de instantáneas con las etiquetas deseadas en el volumen DP de origen para que NetApp Backup and Recovery pueda hacer backup de ellas.


* FlexGroup las copias de seguridad de volúmenes no se pueden trasladar al almacenamiento de archivo.
...
** La política de snapshot aplicada al volumen debe ser una de las políticas reconocidas por NetApp Backup and Recovery, incluyendo diaria, semanal, mensual, etc. La política predeterminada "sm_created" (usada para *Mirror All Snapshots*) no es reconocida y el volumen DP no aparecerá en la lista de volúmenes que se pueden respaldar.
** SVM-DR y la copia de seguridad y recuperación de volúmenes funcionan de forma totalmente independiente cuando la copia de seguridad se realiza desde el origen o desde el destino. La única restricción es que SVM-DR no replica la relación de nube SnapMirror. En el escenario de recuperación ante desastres, cuando la máquina virtual de almacenamiento pasa a estar en línea en la ubicación secundaria, debes actualizar manualmente la relación de nube SnapMirror.


* Soporte de MetroCluster :
+
** Si utilizas ONTAP 9.12.1 GA o una versión posterior, el backup es compatible cuando estás conectado al sistema primario. Toda la configuración de backup se transfiere al sistema secundario para que los backups en la nube continúen automáticamente después de la conmutación de sitios. No necesitas configurar el backup en el sistema secundario (de hecho, no puedes hacerlo).
...
** A partir de ONTAP 9.18.1, las copias de seguridad de volúmenes FlexGroup son compatibles en configuraciones MetroCluster.


* La copia de seguridad ad hoc de un volumen mediante el botón *Backup Now* no es compatible con los volúmenes de protección de datos.
* No se admiten configuraciones SM-BC.
* ONTAP no admite la distribución en ramificación de las relaciones de SnapMirror desde un único volumen a varios almacenes de objetos; por lo tanto, esta configuración no es compatible con NetApp Backup and Recovery.
* El modo WORM/Compliance en un almacén de objetos se admite en Amazon S3, Google Cloud Platform (GCP), Azure y StorageGRID en este momento. Esto se conoce como la función DataLock, y debe gestionarse usando la configuración de NetApp Backup and Recovery, no usando la interfaz del proveedor de la nube.
...
* No es posible restaurar carpetas a partir de instantáneas locales.
* Restauración de archivos y carpetas en una configuración de recuperación ante desastres (DR): después de una conmutación por error, los volúmenes de una máquina virtual de almacenamiento configurada para DR conservan relaciones SnapMirror preexistentes en un estado de ruptura. Los intentos de restaurar archivos o carpetas que crean relaciones RST temporales de SnapMirror son bloqueados por ONTAP porque el volumen de destino ya tiene una relación.
* Explorar y restaurar puede restaurar hasta 100 archivos individuales a la vez.
* Search & Restore puede restaurar un archivo a la vez.
...
== Habilitar la indexación en una configuración de recuperación de desastres

Al activar la indexación en un clúster de SVM de almacenamiento en una configuración de recuperación ante desastres (DR), aparece un error que indica que la operación no está permitida en una SVM que está configurada como destino para SVM DR. A pesar del error, el proceso de backend establece el estado `snapdiff-rpc-server` en "activado" para las SVM de almacenamiento de origen. Después de que se ejecute la programación de la política de DR de SVM de almacenamiento, este estado se propaga a las SVM de almacenamiento de destino.

*Solución*: Espera a que se ejecute la programación de la política de DR de la SVM después del error inicial y luego intenta habilitar la indexación de nuevo.
