...
.. *Registro de host*: Añade un nuevo host de Hyper-V introduciendo el FQDN o la dirección IP del host, las credenciales, el agente de la consola y el número de puerto. Si el agente de la consola no puede resolver el FQDN, utiliza la dirección IP en su lugar. Para clústeres de conmutación por error, introduce la dirección IP de gestión del clúster.
