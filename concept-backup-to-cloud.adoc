...
[role="lead"]
Utiliza esta descripción general para conocer cómo NetApp Backup and Recovery protege tus cargas de trabajo de ONTAP, qué cargas de trabajo y destinos de backup admite, y cómo funcionan las licencias y los costes.
...
La compatibilidad con backup y recuperación ya está integrada en todos los sistemas ONTAP, por lo que no necesitas hardware adicional ni gateways de medios. Esto hace que las operaciones de backup sean simples y rentables. NetApp Console simplifica cualquier implementación de estrategia de backup, incluyendo todo el espectro de variantes de backup 3-2-1, sin necesidad de múltiples gestores de recursos ni personal especializado.
...
** Realiza copias de seguridad de las cargas de trabajo de VMware en proveedores de almacenamiento compatibles.
...
** Consulta link:br-use-oracle-protect-overview.html["Protege las cargas de trabajo de Oracle Database"].
...
** En Google Cloud, se despliega un nuevo bucket, y los https://cloud.google.com/bigquery["Servicios de Google Cloud BigQuery"^] se aprovisionan a nivel de cuenta/proyecto.
...
* Si planeas escanear un archivo de backup en busca de ransomware durante el proceso de restauración de datos de volumen (si activaste la protección Datalock y Ransomware para tus backups en la nube), entonces también incurrirás en costes de salida adicionales de tu proveedor de nube.
...
Para cargas de trabajo de Kubernetes, se te cobrará según el tamaño combinado de todos los volúmenes persistentes.
...
* La tercera opción es adquirir licencias directamente de NetApp. Consulta la sección <<Licencias>> para más detalles.
...
* Google Cloud Storage (solo volúmenes ONTAP y cargas de trabajo de Kubernetes)
* StorageGRID
* ONTAP S3
* Almacenamiento compatible con S3 (solo cargas de trabajo de ONTAP Volumes)
...
* En Azure, NetApp Console utiliza un grupo de recursos nuevo o existente con una cuenta de almacenamiento para el contenedor de blobs. La consola https://docs.microsoft.com/en-us/azure/storage/blobs/anonymous-read-access-prevent["bloquea el acceso público a sus datos de blobs"] de forma predeterminada.
...
== Términos para NetApp Backup and Recovery
