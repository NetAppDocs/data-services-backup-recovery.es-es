...
---
= Descubre la protección de volúmenes de ONTAP con NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Usa NetApp Backup and Recovery para proteger y restaurar los datos de tus volúmenes ONTAP y para archivar backups a largo plazo. Puedes implementar una estrategia 3-2-1 con tres copias de tus datos de origen en dos sistemas de almacenamiento diferentes y una copia en la nube.
...
Tras la activación, NetApp Backup and Recovery crea copias de seguridad incrementales permanentes a nivel de bloque que se almacenan en otro clúster ONTAP y en un almacén de objetos en la nube. Además de tu volumen de origen, tendrás:
...
image:diagram-321-overview-unified.png["Un diagrama que muestra cómo existen los archivos de copia de seguridad en el sistema de origen como instantáneas, como volúmenes replicados en el sistema de almacenamiento secundario y como archivos de copia de seguridad en el almacenamiento de objetos."]

NetApp Backup and Recovery utiliza la tecnología de replicación de datos SnapMirror de NetApp para mantener tus backups sincronizados mediante la creación de copias Snapshot y su transferencia a las ubicaciones de backup.
...
* Configura el cifrado nativo de ONTAP en tránsito usando una clave precompartida (PSK) entre los dos sistemas.
...
* En comparación con https://docs.netapp.com/us-en/data-services-replication/index.html["NetApp Replication"^], la replicación en NetApp Backup and Recovery incluye las siguientes funciones:
+
...
* Analiza los archivos de backup en busca de posibles ataques de ransomware y elimina o reemplaza automáticamente los backups infectados.
...
* Restaura datos desde un punto específico en el tiempo a partir de instantáneas locales, volúmenes replicados o volúmenes de copia de seguridad en almacenamiento de objetos.
...
https://bluexp.netapp.com/cloud-volumes-global-regions["Obtenga más información utilizando el Mapa de regiones globales"^]
...
NetApp Backup and Recovery te permite hacer copias de seguridad de volúmenes ONTAP desde los siguientes sistemas de origen a los siguientes sistemas secundarios y al almacén de objetos en proveedores de nube pública y privada. Las instantáneas residen en el sistema de origen.


NOTE: Wasabi es el único proveedor de almacenamiento de objetos compatible con S3 cualificado en este momento. No se han validado otros proveedores compatibles con S3.
...
| Cloud Volumes ONTAP en Azure  a| 
...
| Cloud Volumes ONTAP en Google  a| 
...
| Almacenamiento de objetos compatible con S3 | Sistema ONTAP local | Sistema ONTAP local 
|===

NOTE: Las referencias a «sistema ONTAP local» incluyen los sistemas FAS, AFF y ONTAP Select.
...
* Si tienes previsto analizar un archivo de copia de seguridad en busca de ransomware durante el proceso de restauración de los datos del volumen (si has activado DataLock y la protección contra ransomware para tus copias de seguridad en la nube), también tendrás que hacer frente a costes adicionales de tráfico de salida por parte de tu proveedor de servicios en la nube.
...
* Las _instantáneas_ residen en el volumen de origen del sistema de origen.
...
+
** En AWS, la consola permite https://docs.aws.amazon.com/AmazonS3/latest/dev/access-control-block-public-access.html["Función de acceso público bloqueado de Amazon S3"^] en el bucket S3.
...
NOTE: La política de snapshot aplicada al volumen debe tener una de las etiquetas que estás usando en tu política de replicación y en tu política de backup a objeto. Si no se encuentran etiquetas coincidentes, no se crearán archivos de backup. Por ejemplo, si quieres crear volúmenes replicados y archivos de backup "semanales", debes usar una política de snapshot que cree snapshots "semanales".
...
Si tu clúster utiliza ONTAP 9.11.1 o una versión posterior, puedes proteger tus copias de seguridad en el almacén de objetos contra el borrado y los ataques de ransomware. Cada política de backup proporciona una sección para la _protección DataLock y ransomware_ que puedes aplicar a tus archivos de backup durante un periodo de tiempo específico: el _periodo de retención_.
...
Actualmente, el soporte está disponible cuando tu destino de backup es Amazon S3, Azure Blob o StorageGRID. Se añadirán otros destinos de proveedores de almacenamiento en futuras versiones.
...
+
Si tu clúster local utiliza ONTAP 9.12.1 o una versión posterior, y tu sistema StorageGRID utiliza 11.4 o una versión posterior, puedes archivar los archivos de backup más antiguos en un almacenamiento de archivo en la nube pública después de un cierto número de días. Actualmente se admite AWS S3 Glacier/S3 Glacier Deep Archive o los niveles de almacenamiento Azure Archive. link:prev-ontap-backup-onprem-storagegrid.html["Obtenga más información sobre cómo archivar archivos de respaldo desde StorageGRID"].



Consulta link:prev-ontap-policy-object-options.html["cómo funciona el archivado de los archivos de backup más antiguos"].
